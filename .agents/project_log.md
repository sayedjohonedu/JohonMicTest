...
- **2026-07-14:** Fixed infinite uncaughtException logging loop on broken stdin/stdout/stderr streams (EPIPE errors) in logger.js. Switched to an asynchronous, non-blocking log queue to avoid blocking the Node event loop and ensure thread-safe rotation/cleanup on Windows. Bumped version to 2.0.1.
