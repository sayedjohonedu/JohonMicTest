/**
 * MicTab Unified Minimal SVG Icon Registry
 * Clean, stroke-consistent (24x24 viewBox, stroke-width: 2, round linecap/linejoin)
 * Reusable across all windows, popups, modals, and toasts.
 */
(function (root, factory) {
  ...
  const ICONS = {
    ...
    key: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21 2-2 2m-1.5 1.5L14 9l-1.5-1.5L11 9l-1.5-1.5L8 9l-1.5-1.5L5 9l-3 3v4h4l1-1v-2h2v-2h2l1.5-1.5L19 5.5m2-3.5a2.121 2.121 0 0 1 0 3l-1.5 1.5-3-3L18 2a2.121 2.121 0 0 1 3 0Z"/></svg>`,
    ...
    lens: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="4"/><line x1="4.93" y1="4.93" x2="9.17" y2="9.17"/><line x1="14.83" y1="14.83" x2="19.07" y2="19.07"/><line x1="14.83" y1="9.17" x2="19.07" y2="4.93"/><line x1="4.93" y1="19.07" x2="9.17" y2="14.83"/></svg>`,
    ...
    record: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3" fill="currentColor"/></svg>`,
    ...
    'git-merge': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M6 9v12"/><path d="M6 9a9 9 0 0 0 9 9h3"/></svg>`,
  ...
  /**
   * Helper to render an SVG icon string with optional size/class/color overrides
   */
  function render(name, options = {}) {
    ...
    return raw
      .replace('<svg', `<svg width="${size}" height="${size}" class="${className}" style="color:${color};"`)
      .replace(/stroke-width="[^"]*"/, `stroke-width="${strokeWidth}"`);
  }

  /**
   * Helper to populate an element by selector with an icon
   */
  function inject(selector, name, options = {}) {
    const el = typeof selector === 'string' ? document.querySelector(selector) : selector;
    if (el) {
      el.innerHTML = render(name, options);
    }
  }

  // ── Safe browser fallback for testing in standard browsers ──────────
  if (typeof window !== 'undefined' && !window.junoAPI) {
    window.junoAPI = {
      openUrl: (url) => window.open(url, '_blank'),
      closeLensLockedPopup: () => window.close(),
      closeScreenRecorderLockedPopup: () => window.close(),
      closeTranslatorLockedPopup: () => window.close(),
      closeWhisperApiLockedPopup: () => window.close(),
      closeOfflineLockedPopup: () => window.close(),
      closeAiTrialPopup: () => window.close(),
      closeLicensePopup: () => window.close(),
      closeWordLimitPopup: () => window.close(),
      closeLicenseCelebration: () => {},
      dismissUpdateReminder: () => window.close(),
      downloadUpdate: () => console.log('Downloading update…'),
      installUpdate: () => console.log('Installing update…'),
      getLicenseInfo: () => Promise.resolve({ licenseActivatedDate: '2026-02-15T00:00:00.000Z' }),
      onUpdateReminderInfo: (cb) => cb({ version: '2.1.0' }),
      onUpdateStatus: (cb) => {},
