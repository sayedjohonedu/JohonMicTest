  ...
  ipcMain.on('open-url', (event, url) => {
    try {
      ...
      if (['http:', 'https:', 'mailto:'].includes(parsedUrl.protocol)) {
        ...
        console.warn(`[Security] Blocked attempt to open potentially unsafe URL protocol: ${parsedUrl.protocol}`);
      }
    } catch (err) {
      console.error('[Security] Failed to parse URL for open-url:', err);
    }
  });
