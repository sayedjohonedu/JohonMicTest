...
let _timer        = null;
let _lastText     = '';
let _lastImgHash  = '';   // SHA-256 hash of image buffer — avoids expensive readImage()
...
function start(onNewEntry) {
  ...
  // We use readBuffer() and a SHA-256 hash to detect changes without decoding the image.
...
function _checkImage() {
  // Optimization: clipboard.readImage() is computationally expensive.
  // In polling loops, checking clipboard.availableFormats() is insufficient because
  // it causes redundant decodings if an image remains on the clipboard.
  // Instead, use clipboard.readBuffer(format) to read raw image bytes and hash them
  // (via native crypto SHA-256) to detect changes, avoiding continuous readImage() polling.
  const formats = clipboard.availableFormats();
  const imageFormat = formats.find(f => f.startsWith('image/'));
  if (!imageFormat) {
  ...
  // Only perform the expensive readImage() when we are sure the image is new
