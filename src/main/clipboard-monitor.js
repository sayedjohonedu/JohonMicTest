...
let _timer        = null;
let _lastText     = '';
let _lastImgHash  = '';   // SHA-256 hash of last image buffer — lightweight change indicator
...
function start(onNewEntry) {
  ...
  // Optimization: clipboard.readImage() is computationally expensive and causes redundant decodings
  // if an image remains on the clipboard. We use clipboard.readBuffer(format) to read raw image bytes
  // and hash them to detect changes without decoding.
  const formats = clipboard.availableFormats();
  const imgFormat = formats.find(f => f.startsWith('image/'));
  if (imgFormat) {
    const buf = clipboard.readBuffer(imgFormat);
    _lastImgHash = buf.length ? crypto.createHash('sha256').update(buf).digest('hex') : '';
  } else {
    _lastImgHash = '';
  }
...
function _checkImage() {
  ...
  // Optimization: clipboard.readImage() is computationally expensive and causes redundant decodings
  // if an image remains on the clipboard. We use clipboard.readBuffer(format) to read raw image bytes
  // and hash them to detect changes, avoiding continuous readImage() polling.
  const buf = clipboard.readBuffer(imgFormat);
  const hash = buf.length ? crypto.createHash('sha256').update(buf).digest('hex') : '';
  if (!hash || hash === _lastImgHash) return;
