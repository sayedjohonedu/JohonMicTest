...
let _timer        = null;
let _lastText     = '';
let _lastImgHash  = '';   // SHA-256 of raw image bytes — lightweight change indicator
...
function start(onNewEntry) {
  ...
  // Avoid blocking main thread on startup by reading raw bytes and hashing them instead of decoding.
...
function _checkImage() {
  // Optimization: readImage() decoding is expensive. Read raw buffer and hash it to
  // check for changes before doing the full decoding.
  const formats = clipboard.availableFormats();
  const imageFormat = formats.find(f => f.startsWith('image/'));
  if (!imageFormat) {
